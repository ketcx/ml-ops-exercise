    ...
    def __init__(
        ...
        if self.mini_batch_size is not None:
            if not isinstance(self.mini_batch_size, PipelineParameter):
                if not isinstance(self.mini_batch_size, (int, str)):
        ...
        if isinstance(self.compute_target, AmlCompute) and not isinstance(self.node_count, PipelineParameter):
            if self.node_count > self.compute_target.scale_settings.maximum_node_count or self.node_count <= 0:
                raise ValueError(
                    "Parameter node_count must be between 1 and max_nodes {}".format(
                        self.compute_target.scale_settings.maximum_node_count
                    )
                )
