...
def ml_workspace_create_resources(auth, client, resource_group_name, workspace_name,
                                  location, vnet_location, subscription_id,
                                  friendly_name=None,
                                  ...
                                  exist_ok=False, show_output=True, sku='basic', tags=None):
    """
    ...
    orchestrator = WorkspaceArmDeploymentOrchestrator(auth, resource_group_name, location, subscription_id,
                                                      workspace_name, storage=storage_account, keyVault=key_vault,
                                                      containerRegistry=containerRegistry, adbWorkspace=adbWorkspace,
                                                      appInsights=app_insights, cmkKeyVault=cmk_keyvault,
                                                      resourceCmkUri=resource_cmk_uri, hbiWorkspace=hbi_workspace,
                                                      sku=sku, tags=tags)
    ...
    if private_endpoint_config:
        try:
            create_workspace_private_endpoint(auth, resource_group_name, vnet_location,
                                              subscription_id, workspace_name, private_endpoint_config,
                                              private_endpoint_auto_approval, tags, show_output)
        except Exception:
            _delete_resources(auth, subscription_id, resource_group_name, storage_account=storage_account,
                              app_insights=app_insights, key_vault=key_vault,
                              storage_name=orchestrator.storage_name, insights_name=orchestrator.insights_name,
...
def create_workspace_private_endpoint(auth, resource_group_name, location, subscription_id, workspace_name,
    ...
    orchestrator = PrivateEndPointArmDeploymentOrchestrator(auth, resource_group_name, location,
                                                            subscription_id, workspace_name, private_endpoint_config,
                                                            private_endpoint_auto_approval, tags=tags)
    orchestrator.deploy_private_endpoint(show_output=show_output)

    # Don't delete workspace if private endpoint creation fails.
    # Throw a warning. User has an option to add private endpoint to workspace later.
    if orchestrator.error is not None:
        error_message = orchestrator.error.message
        raise WorkspacePrivateEndpointException("The workspace private endpoint resource"
...
def ml_workspace_update(client, resource_group_name, workspace_name, friendly_name=None,
                        description=None, tags=None, image_build_compute=None, sku=None,
    ...
    if friendly_name is None and description is None and \
       ...
       service_managed_resources_settings is None:
        return workspace
    else:
    ...
    workspace = WorkspacesOperations.update(client, resource_group_name, workspace_name, params)
    return workspace
