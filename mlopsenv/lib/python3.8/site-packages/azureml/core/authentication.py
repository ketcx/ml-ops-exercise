...
class InteractiveLoginAuthentication(AbstractAuthentication):
    ...
    def _get_ambient(self):
        ...
        if not ambient_auth:
            # try MSI
            ambient_auth = MsiAuthentication._initialize_msi_auth(tenant_id_to_check=self._tenant_id)

        if not ambient_auth:
            # try SP from env vars
            ambient_auth = ServicePrincipalAuthentication._initialize_sp_auth(self._tenant_id)

        if not ambient_auth:
            # For arcadia environment
            ambient_auth = _ArcadiaAuthentication._initialize_arcadia_auth(tenant_id_to_check=self._tenant_id)
        ...
        if not ambient_auth:
            ...
            ambient_auth = ArmTokenAuthentication._initialize_arm_token_auth(self._tenant_id)
...
class ServicePrincipalAuthentication(AbstractAuthentication):
    ...
    @staticmethod
    def _initialize_sp_auth(tenant_id):
        try:
            ...
            return ServicePrincipalAuthentication(tenant_id, service_principal_id, service_principal_password)
        except Exception as e:
            module_logger.debug(e)
...
class AzureMLTokenAuthentication(AbstractAuthentication):
    ...
    def __init__(self, azureml_access_token, expiry_time=None, host=None, subscription_id=None,
                 resource_group_name=None, workspace_name=None, experiment_name=None, run_id=None, user_email=None, cloud='AzureCloud'):
...
class MsiAuthentication(AbstractAuthentication):
    ...
    Only system identity is supported. User-assigned MSIs are not supported.
    ...
    @staticmethod
    def _initialize_msi_auth(tenant_id_to_check=None):
        ...
        try:
            ...
            if "MSI_ENDPOINT" in os.environ or os.path.isfile(nbvm_file_path):
                ...
                msi_auth = MsiAuthentication(cloud=None)
                # make sure we can get an arm token through msi to validate the auth object.
...
class _ArcadiaAuthentication(ArmTokenAuthentication):
    ...
    @staticmethod
    def _initialize_arcadia_auth(tenant_id_to_check=None):
        ...
        try:
            if _ArcadiaAuthentication._is_arcadia_environment():
                arcadia_auth = _ArcadiaAuthentication()
