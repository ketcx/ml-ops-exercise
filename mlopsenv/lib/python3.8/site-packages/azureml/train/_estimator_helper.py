...
from azureml._base_sdk_common.utils import convert_dict_to_list, merge_list, \
    list_remove_none_object, list_remove_empty_strings
...
def _init_run_config(estimator, source_directory, compute_target, vm_size=None, vm_priority=None, entry_script=None,
    ...
    arguments = _get_arguments(script_params, allow_empty_strings=True)
...
def _get_arguments(script_params, allow_empty_strings=False):
    from azureml.data.azure_storage_datastore import AbstractAzureStorageDatastore
    ...
    params_list = list_remove_none_object(convert_dict_to_list(script_params_copy))
    if not allow_empty_strings:
        params_list = list_remove_empty_strings(params_list)
