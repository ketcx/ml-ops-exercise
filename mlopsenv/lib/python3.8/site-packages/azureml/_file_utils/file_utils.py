...
def download_file(source_uri, path=None, max_retries=5, stream=True, protocol="https", session=None,
                  _validate_check_sum=False, max_concurrency=8):
    """
    ...
    if is_source_uri_matches_storage_blob(source_uri):
        ...
        def exec_func():
            metadata_dict = blob_service.get_blob_to_path(container_name=container_name,
                                                          blob_name=blob_name,
                                                          file_path=path,
            ...
            if(content_length != file_size):
                raise AzureMLException('downloaded file did not match blob: {} bytes downloaded but {} bytes ' +
                                       'present in blob.'.format(file_size, content_length))
    ...
    if is_source_uri_matches_storage_blob(source_uri):
        ...
        def exec_func():
            if download_to_bytes:
                blob = blob_service.get_blob_to_bytes(container_name=container_name,
                                                      blob_name=blob_name,
                                                      max_connections=max_concurrency,
                                                      validate_content=_validate_check_sum)
            else:
                blob = blob_service.get_blob_to_text(container_name=container_name,
                                                     blob_name=blob_name,
                                                     max_connections=max_concurrency,
                                                     validate_content=_validate_check_sum,
                                                     encoding=encoding)
...
def _request_file_with_retry(source_uri, handle_response, max_retries, stream, session):
    ...
    return _retry(exec_func, clean_up_func, max_retries=max_retries, exceptions=exceptions)
